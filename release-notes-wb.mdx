...
该版本废弃了白板数据中心（`region`）原有的英国伦敦数据中心（`gb-lon`），并新增了欧洲法兰克福数据中心（`eu`），服务区覆盖欧洲。如果需要使用该数据中心，请在升级到该版本后更新代码，并在控制台-互动白板服务配置中启用该数据中心。

<VersionTitle icon="/img/icons/version-bugfix.png">问题修复</VersionTitle>

该版本修复了在 Windows 平台上使用白板 Web 应用时，`shift` 按键判定有误，导致铅笔工具切换为画直线后无法恢复。

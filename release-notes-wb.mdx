...
新增 `screenshotToCanvasAsync` 方法，用于等待目标场景的图片加载完成后生成屏幕快照，写入指定的渲染上下文中。该功能目前处于实验阶段，如果你在使用中遇到了问题，请[联系技术支持](https://ticket.shengwang.cn/)。
...
- 新增属性 `room.useNativeClipboard`，用于设置是否使用原生剪贴板。启用后，快捷键里的复制和粘贴将失效，改为使用原生的 `copy` 和 `paste` 事件。
- `DisplayerCallbacks` 新增 `onBackgroundError` 回调，用于报告背景图加载失败。
- `RoomCallbacks` 新增 `onObjectsLimit` 回调，用于报告白板中的元素数量超过限制。当到达限制时前端会抛出警告和事件，超出限制时无法再添加元素，房间状态会被强制回退，且后端会强制踢出用户。

...
为提升 API 调用的连通性，该版本新增了以下备选 API 域名，SDK 初始化时将自动选择：
...
- 新增 `onConnection` 事件，报告 WebSocket 已连接，正在下载当前方间的白板快照数据，恢复房间状态中。你可以通过该回调事件提示用户白板方间加载中。
...
- `ConstructRoomParams` 新增 `commandWheelToZoom` 属性 ，用于实现按住 `command` 键 + 滚动鼠标滚轮以缩放白板。
- 优化了浮动条中“透明”填充色的按钮样式。

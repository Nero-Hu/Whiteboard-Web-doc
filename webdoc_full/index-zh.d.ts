 ...
 */
export declare function previewPPT(
...
export declare interface CursorAdapter {
   ...
   */
  readonly onMovingCursor:
    | ((cursor: Cursor, positionX: number, positionY: number) => void)
    | undefined;
}
 ...
 * @param plugins 创建的插件。
 */
export declare function createPlugins<C_MAP extends Object>(
  plugins: Readonly<{
    [K: string]: Plugin<any>;
  }>
): Plugins<C_MAP>;
 ...
 */
export declare type Plugin<C = {}, T = any> = {
  /**
 ...
 * 不可见插件。
 */
export declare type InvisiblePluginClass<
  K extends string,
  A extends Object,
  P extends InvisiblePlugin<A> = InvisiblePlugin<A>
> = {
  /**
   * 插件的类型。
   */
...
export declare type HotKey =
  | string
  | HotKeyDescription
  | ReadonlyArray<string | HotKeyDescription>
  | HotKeyChecker;
 ...
 * - `false`：不是指定的快捷键。
 */
export declare type HotKeyChecker = (
  event: HotKeyEvent,
  kind: KeyboardKind
) => boolean;
 ...
 */
export declare interface Displayer<
  CALLBACKS extends DisplayerCallbacks = DisplayerCallbacks
> {
  /**
...
export declare interface Room extends Displayer {
  ...
  /**
   * 取消通过 {@link syncBlockTimestamp} 方法设置本地显示远端白板内容的 Unix 时间戳。
   * @since 2.13.8
   */
   ...
   *
   * @param scenePath 需要导出的场景路径。
   *
   * @returns 导出场景内容的 Blob 对象。
   */
   ...
   * @since v2.14.5
   *
   * 你可以调用该方法将通过 {@link exportScene} 导出的场景内容导入指定的场景目录中。
   *
   * @param dir 场景要导入的场景目录。确保该场景目录的 {@link ScenePathType} 不为 `conflict`；否则，场景将无法导入。
   * 你可以通过 {@link scenePathType} 查询路径类型。
   *
   * @param payload 场景内容的 Blob 对象。
   *
   * @returns 方法调用成功后，返回 {@link SceneDefinition} 对象，其中包含导入场景的 `name`。导入场景的路径即为 `dir`+ `name`。
   * 如果需要修改场景路径，可以调用 {@link moveScene}。
   *
   */
...
//TODO CT review
/**
 * 轮询 PPT 转换进度的响应内容。（服务端返回的 HTTP 响应？）
 */
...
export declare type PreviewConfig = {
  /** //TODO 这个具体指的是哪里的菜单？
   * 国际化白板上方的菜单栏语言。详见 {@link International}。
   */
 ...
 * 组件插件的渲染。
 */
export declare type RenderPlugin<C = {}, T = any> = {
  /**
   * 组件插件类型，为该组件插件的唯一识别符。
   */
  kind: string;
...
/**
 * //TODO 这个的意思是，如果我给 prePage 传入的 string 是 “previous”，菜单栏就会显示成“previous”？
...
/** 用于打印预览 PPT 的日志。 */
export declare type LoggerPrinter<
  C = {
    [key: string]: any;
  }
> = {

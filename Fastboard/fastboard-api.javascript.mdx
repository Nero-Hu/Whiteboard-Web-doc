...
该方法仅适用于使用 [`insertDocs`](#insertdocs1) 的三个重载方法插入的文档。
...
不传入 `force` 直接调用该方法时，铅笔工具将在画实线和画虚线之间切换。

**参数**

- `force`：Boolean。（可选）是否切换为虚线：
  - `true`：切换为画虚线。
  - `false`：切换为画实线。
...
<H3 className="anchor index-api-insertdocs" id="insertdocs1">insertDocs [1/3]</H3>

```typescript
insertDocs(title: string, status: ConversionResponse): Promise<string | undefined>;
```

在白板子窗口中插入并展示文档。该方法适用于[旧版文档转换服务](/doc/whiteboard/javascript/fastboard-sdk/advanced-features/convert-files-old)。
...
<H3 className="anchor index-api-insertdocs" id="insertdocs2">insertDocs [2/3]</H3>
...
<H3 className="anchor index-api-insertdocs" id="insertdocs3">insertDocs [3/3]</H3>

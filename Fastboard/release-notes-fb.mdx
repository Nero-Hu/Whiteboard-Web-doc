...
- 铅笔工具：新增 [`toggleDottedLine`](/api-ref/whiteboard/javascript/fastboard-api#toggledottedline) 方法，用于切换铅笔工具的画线样式。
- 铅笔橡皮工具：新增 [`setPencilEraserSize`](/api-ref/whiteboard/javascript/fastboard-api#setpencilerasersize) 方法，用于修改铅笔橡皮的大小。

<VersionTitle icon="/img/icons/version-improve.png">改进</VersionTitle>

该版本进行了如下改进：

- [`insertImage`](/api-ref/whiteboard/javascript/fastboard-api#insertimage) 方法新增参数 `crossOrigin`，用于设置是否对插入的图片启用跨域资源共享 (CORS, Cross-Origin Resource Sharing) 。
- 新增 [`jumpPage`](/api-ref/whiteboard/javascript/fastboard-api#jumppage) 方法，用于跳转至指定白板页。相比之前修改 `FastboardApp` 的 `sceneIndex` 属性的方式更加便捷。

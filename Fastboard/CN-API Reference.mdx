...
<Admonition type="caution" title="注意">自 v0.3.6 起不推荐使用该方法，请改用 <a href="#createui"><code>createUI</code></a>。</Admonition>
挂载白板应用到 HTML 元素上。
...
<h3 className="anchor index-api-insertdocs" id="insertdocs">insertDocs [1/2]</h3>
...
<h3 className="anchor index-api-insertdocs" id="insertdocs">insertDocs [2/2]</h3>
...
<Admonition type="caution" title="注意">该方法自 v0.3.6 起废弃，请改用 `fastboard.manager.addApp({ kind: 'Monaco' })` 实现。</Admonition>
插入代码编辑器。
...
<Admonition type="caution" title="注意">该方法自 v0.3.6 起废弃，请改用 `fastboard.manager.addApp({ kind: 'Countdown' })` 实现。</Admonition>
插入计时器。
...
<Admonition type="caution" title="注意">该方法自 v0.3.6 起废弃，请改用 `fastboard.manager.addApp({ kind: 'GeoGebra' })` 实现。</Admonition>
插入 GeoGebra 数学计算器。
...
- `scenes`: SceneDefinition[]。白板场景内容，即待展示的文档。由多个 object 组成，每个 object 表示一页文件，必须包含如下参数：
  ...
  - `ppt`：PptDescription。PPT 描述，包含以下属性：
    ...
    - `src`：String。该页文件的 URL 地址，即[查询文档转换任务进度](/api-ref/whiteboard/restful/restful-wb/operations/get-v5-projector-tasks-uuid) 返回的响应包体中 `conversionFileUrl` 字段的值。
- `title`: （可选）String。窗口标题。

...
<h2 className="anchor index-auto-group index-class-dispatchdocsevent" id="dispatchdocsevent">dispatchDocsEvent</h2>
...
该方法仅适用于使用 [`insertDocs`](#insertdocs) 方法插入的文档。

**参数**

- `fastboard`：[`FastboardApp`](#createfastboard) 或 [`WindowManager`](https://github.com/netless-io/window-manager/blob/master/docs/api.md#mount) 对象。发送事件的对象。
- `event`：String。需要触发的事件类型，取值如下：
  - `prevPage`：上一页。
  - `nextPage`：下一页。
  - `prevStep`：上一步。向静态文档发送此事件效果与 `prevPage` 相同。
  - `nextStep`：下一步。向静态文档发送此事件效果与 `nextPage` 相同。
  - `jumpToPage`：跳转至页码。发送此事件时，需要传入 `options` 以指定页码。
...
<h3 className="anchor index-api-insertdocs" id="insertdocs">insertDocs [1/3]</h3>

```typescript
insertDocs(title: string, response: ConversionResponse): Promise<string | undefined>;
```

在白板子窗口中插入并展示文档。该方法适用于[旧版文档转换服务](/doc/whiteboard/web/fastboard-sdk/advanced-features/convert-files-old)。

成功发起文档转换任务并调用[查询文档转换任务进度（旧版）](https://docportal.shengwang.cn/cn/whiteboard/whiteboard_file_conversion_old?platform=RESTful#%E6%9F%A5%E8%AF%A2%E8%BD%AC%E6%8D%A2%E4%BB%BB%E5%8A%A1%E7%9A%84%E8%BF%9B%E5%BA%A6%EF%BC%88get%EF%BC%89)获取文档转换结果后，可以调用该方法并传入获取的文档转换结果，SDK 会自动创建子窗口，插入并分页展示转换后的文档。

**参数**

- `title`：窗口标题。
- `response`：[查询文档转换任务进度（旧版）](https://docportal.shengwang.cn/cn/whiteboard/whiteboard_file_conversion_old?platform=RESTful#%E6%9F%A5%E8%AF%A2%E8%BD%AC%E6%8D%A2%E4%BB%BB%E5%8A%A1%E7%9A%84%E8%BF%9B%E5%BA%A6%EF%BC%88get%EF%BC%89)请求成功时返回的响应包体。

**返回**

方法调用成功时，返回插件的标识符。

<h3 className="anchor index-api-insertdocs" id="insertdocs">insertDocs [2/3]</h3>

```typescript
insertDocs(filename: string, response: ProjectorResponse): Promise<string | undefined>;
```

在白板子窗口中插入并展示文档。该方法适用于[新版文档转换服务](/doc/whiteboard/web/fastboard-sdk/advanced-features/convert-files)。

成功发起文档转换任务并调用[查询文档转换任务进度](/api-ref/whiteboard/restful/restful-wb/operations/get-v5-projector-tasks-uuid)获取文档转换结果后，可以调用该方法并传入获取的文档转换结果，SDK 会自动创建子窗口，插入并分页展示转换后的文档。

**参数**

- `title`：窗口标题。
- `response`：[查询文档转换任务进度](/api-ref/whiteboard/restful/restful-wb/operations/get-v5-projector-tasks-uuid)请求成功时返回的响应包体。

**返回**

方法调用成功时，返回插件的标识符。

<h3 className="anchor index-api-insertdocs" id="insertdocs">insertDocs [3/3]</h3>
...
成功[发起文档转换任务](/api-ref/whiteboard/restful/restful-wb/operations/post-v5-projector-tasks)后， 可以调用该方法，传入转换后文档的相关参数，SDK 会自动创建子窗口，插入并分页展示转换后的文档。


<Admonition type="info" title="信息">
 对比 `insertDocs [1/3]` 和 `insertDocs [2/3]`，`insertDocs [3/3]` 需要手动传入少量参数，而不是直接传入[查询文档转换任务进度](/api-ref/whiteboard/restful/restful-wb/operations/get-v5-projector-tasks-uuid)的响应包体。
</Admonition>
...
- `appId`: （可选）String。指定事件发送的文档。如果不传入，则为当前焦点所在的文档。
...
- `sdkConfig`: （必需）Object。`WhiteWebSdk` 实例的配置，必须传入以下参数：
  ...
  - `region`：String。数据中心，必须与[创建房间](/api-ref/whiteboard/restful/restful-wb/operations/post-v5-rooms)时设置的数据中心一致。取值如下：
...
- `joinRoom`: Object。（必需）加入房间的参数设置，必须传入以下参数：
  ...
  - `roomToken`: String 型。房间的 Room Token，用于加入房间时的用户鉴权。可以通过以下方式获取：
    ...
    - 在 app 服务端用代码生成，详见[在 app 服务端生成 Token](/doc/whiteboard/web/fastboard-sdk/basic-features/generate-token#在-app-服务端生成-token)。
...
- `scenePath`: String。白板场景路径，建议填 `"/pptx/uuid"`，其中 `uuid` 为文档转换任务的 UUID，即[发起文档转换任务](/api-ref/whiteboard/restful/restful-wb/operations/post-v5-projector-tasks) 请求成功时响应包体中 `uuid` 参数的值。
- `taskId`: String。文档转换任务的 UUID，即[发起文档转换任务](/api-ref/whiteboard/restful/restful-wb/operations/post-v5-projector-tasks) 请求成功时响应包体中 `uuid` 参数的值。
...
- `scenePath`: String。白板场景路径，建议填 `"/pdf/uuid"`，其中 `uuid` 为文档转换任务的 UUID，即[发起文档转换任务](/api-ref/whiteboard/restful/restful-wb/operations/post-v5-projector-tasks) 请求成功时响应包体中 `uuid` 参数的值。
- `scenes`: SceneDefinition[]。白板场景内容，即待展示的文档。由多个 object 组成，每个 object 表示一页文件，必须包含如下参数：
  ...
  - `ppt`：PptDescription。PPT 描述，包含以下属性：
    ...
    - `src`：String。该页文件的 URL 地址，即[查询文档转换任务进度](/api-ref/whiteboard/restful/restful-wb/operations/get-v5-projector-tasks-uuid) 返回的响应包体中 `images` 字段下 `url` 的值。
- `title`: （可选）String。窗口标题。

...
- `appId`：String。互动白板项目的 AppIdentifier。详见[获取互动白板项目的安全密钥](/doc/whiteboard/javascript/whiteboard-sdk/get-started/enable-service#获取互动白板项目的安全密钥)。
...
- `token`：String。房间的 Room Token，用于加入房间时的用户鉴权。可以通过以下方式获取：
  ...
  - 在 app 服务端用代码生成，详见[在 app 服务端生成 Token](/doc/whiteboard/javascript/whiteboard-sdk/basic-features/generate-token#在-app-服务端生成-token)。
...
- `converterType`: 枚举。文档转换服务的版本，取值如下：
  - `Projector`: 新版。详见[新版文档转换服务](/doc/whiteboard/javascript/whiteboard-sdk/advanced-features/convert-files)。
  - `WhiteboardConverter`: 旧版（默认值）。详见[旧版文档转换服务](/doc/whiteboard/javascript/whiteboard-sdk/advanced-features/convert-files-old)。
